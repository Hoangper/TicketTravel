...
const ButtonCustom: React.FC<ButtonProps> = (props) => {
  return (
      <TouchableOpacity
      onPress={props.onPress} style={StyleSheet.flatten([styles.button,props.buttonstyle])}>
        <Text style={StyleSheet.flatten([styles.buttontext,props.textstyle])}>{props.content}</Text>
      </TouchableOpacity>
  )
}
